...
- `src/events.ts` — the `library/inject` + `library/purge` `SessionEventMap` members (declaration merging), the payload types, and the adaptive append gate: known-vocabulary hosts get the events, `ignorable`-envelope builds get them with the marker, and envelope-less builds (0.1.1-rc.2, 0.1.2-alpha.1, which fail closed on unknown event types at read) get no append.
