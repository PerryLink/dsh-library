...
| 方面 | 状态 |
|---|---|
| Harness | DeepSeek Harness `0.1.2-alpha.3`（声明兼容 `0.1.2-alpha.3`） |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **会话日志**：`library/inject`（id、查询、切片 id、页大小）与 `library/purge`（判定）是仅日志审计事件 —— 模型可见的注入页可从中重建。写入受宿主门控：已知类型集合覆盖该词汇的宿主直接写入；带 `ignorable` 信封的构建加标记写入；无信封构建（0.1.1-rc.2、0.1.2-alpha.3）跳过写入 —— 此时已记录的 `tool/call` + `tool/result` 事件仍是可重建的审计链。
